...
export default function Bookings() {
  ...
  const sortBookings = (arr) => {
    return [...arr].sort((a, b) => {
      ...
      const nameA = (a.clientName || a.cleaner?.name || "").toLowerCase();
      const nameB = (b.clientName || b.cleaner?.name || "").toLowerCase();
      if (nameA < nameB) return -1;
      if (nameA > nameB) return 1;

      // Secondary sort: Chronological by Service Date
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(b =>
        (b.clientName?.toLowerCase().includes(searchTerm.toLowerCase()) || false) ||
        (b.cleaner?.name?.toLowerCase().includes(searchTerm.toLowerCase()) || false) ||
    ...
    if (filter.professional) {
      filtered = filtered.filter(b => String(b.cleanerId) === filter.professional);
    }

    // Filter by service type
    if (filter.serviceType) {
      filtered = filtered.filter(b => b.serviceType === filter.serviceType);
    }
    ...
    if (filter.startDate) {
      ...
      filtered = filtered.filter(b => new Date(b.serviceDate) >= start);
    }
    if (filter.endDate) {
      const end = new Date(filter.endDate);
      end.setHours(23, 59, 59, 999); // Normalize to end of day
      filtered = filtered.filter(b => new Date(b.serviceDate) <= end);
    }
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 text-left">
            <thead className="bg-gray-50/50">
              <tr>
                <th scope="col" className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Cliente</th>
                <th scope="col" className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Data e Hora</th>
                <th scope="col" className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Profissional</th>
                <th scope="col" className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Serviço</th>
              ...
              {bookings.map((b) => (
                <tr key={b.id} className="hover:bg-gray-50/50 transition-colors group">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        ...
                        <div className="text-sm text-gray-500">{b.clientEmail}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {b.serviceType === 'residential' ? 'Residencial' : 'Comercial'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <select
                      value={b.status}
